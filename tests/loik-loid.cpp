...
struct ProblemSetupFixture {
  ...
  ProblemSetupFixture() {
    ...
    v_ref = Motion::Zero();
    active_task_constraint_ids.push_back(
        static_cast<Index>(robot_model.njoints - 1));
...
BOOST_FIXTURE_TEST_CASE(test_problem_setup, ProblemSetupFixture) {
  ...
  FirstOrderLoik LoikSolver_test{max_iter_test,
                                 tol_abs_test,
                                 tol_rel_test,
                                 tol_primal_inf_test,
                                 tol_dual_inf_test,
                                 rho_test,
                                 mu_test,
                                 mu_equality_scale_factor_test,
                                 mu_update_strat_test,
                                 num_eq_c_test,
                                 eq_c_dim_test,
                                 robot_model_test,
                                 ikid_data_test,
  ...
  LoikSolver_test.Solve(q_test, H_ref_test, v_ref_test,
                        active_task_constraint_ids_test, Ais_test, bis_test,
                        lb_test, ub_test);
...
BOOST_FIXTURE_TEST_CASE(test_loik_solve_split, ProblemSetupFixture) {
  ...
  LoikSolver_test.SolveInit(q, H_ref, v_ref, active_task_constraint_ids, Ais,
                            bis, lb, ub);
...
// const int NBT = 1;
  ...
  LoikSolver.SolveInit(q, H_ref, v_ref, active_task_constraint_ids, Ais, bis,
                       lb, ub);
...
BOOST_FIXTURE_TEST_CASE(test_1st_order_loik_optimized_correctness,
                        ProblemSetupFixture) {
  ...
  for (const auto &idx : ikid_data_test.joint_full_range) {
    ...
    BOOST_CHECK(
        (ikid_data_test.pis[idx].toVector()).isApprox(ikid_data.pis[idx]));
    BOOST_CHECK((ikid_data_test.pis_aba[idx].toVector())
                    .isApprox(ikid_data_test.pis[idx].toVector()));
  }
  ...
  for (const auto &idx : ikid_data_test.joint_range) {
    ...
    BOOST_CHECK((jmodel.jointVelocitySelector(ikid_data_test.R))
                    .isApprox(ikid_data.Ris[idx]));
  ...
  for (const auto &idx : ikid_data_test.joint_range) {
    ...
    BOOST_CHECK(
        (ikid_data_test.fis[idx].toVector()).isApprox(ikid_data.fis[idx]));
  }
  ...
  BOOST_CHECK(LoikSolver.get_primal_residual() ==
              LoikSolver_test.get_primal_residual());
  BOOST_CHECK(LoikSolver.get_primal_residual_vec().isApprox(
      LoikSolver_test.get_primal_residual_vec()));
  BOOST_CHECK(LoikSolver.get_dual_residual() ==
              LoikSolver_test.get_dual_residual());
  BOOST_CHECK(LoikSolver.get_dual_residual_vec().isApprox(
      LoikSolver_test.get_dual_residual_vec()));
  std::cout << "primal residual from LoikSolver: "
            << LoikSolver.get_primal_residual() << std::endl;
  std::cout << "primal residual from LoikSolver_test: "
            << LoikSolver_test.get_primal_residual() << std::endl;
  std::cout << "dual residual from LoikSolver: "
            << LoikSolver.get_dual_residual() << std::endl;
  std::cout << "dual residual from LoikSolver_test: "
            << LoikSolver_test.get_dual_residual() << std::endl;
  std::cout << "dual residual v from LoikSolver: "
            << LoikSolver.get_dual_residual_v() << std::endl;
  std::cout << "dual residual v from LoikSolver_test: "
            << LoikSolver_test.get_dual_residual_v() << std::endl;
  std::cout << "dual residual nu from LoikSolver: "
            << LoikSolver.get_dual_residual_nu() << std::endl;
  std::cout << "dual residual nu from LoikSolver_test: "
            << LoikSolver_test.get_dual_residual_nu() << std::endl;
