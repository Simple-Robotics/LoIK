...
namespace loik
{
  ...
  , pinocchio::NumericalBase<IkIdDataTypeOptimizedTpl<_Scalar, _Options, JointCollectionTpl>>
  {
    ...
    /// \brief Dense vectorized version of a joint tangent vector (e.g. velocity, acceleration, etc), q_dot.
    ...
    /// \brief link spatial velocity expressed in link's local frame from the previous solver iteration
    ...
    /// \brief initialized to 'His' but acts as a container for tmp quantities that gets overwritten during LOIK bwdpass,
    /// after bwdpass, it becomes Pi * Hi, analogous to 'data.Yaba'
    ...
    /// \brief quadratic penalty term associated with nu_i introduced by inequality constraints on nu_i through ADMM, equivalent to armature
    DVec R;

    /// \brief affine terms associated with nu_i introduced by inequality constraints on nu_i through ADMM, type joint space "force"
    ///        analogous to 'data.u'
    DVec r;
    ...
    ///
    /// \brief untility member, all joint index collected into a std::vector, including the universe joint
    ///        should be moved to Model
    ///
    IndexVector joint_full_range;

    ///
    /// \brief untility member, all joint index collected into a std::vector, excluding the universe joint
    ///        should be moved to Model
    ///
    ...
    ///
    /// \brief Ai.transpose * yi  ; updated in the solver after dual variables (yis) udpate (i.e. in DualUpdate).
    ...
    ///
    /// \brief delta of fis_diff_plus_Aty from previous iteration, needed for primal infeasibility checking
  ...
  inline bool checkIkIdData(const pinocchio::ModelTpl<Scalar,Options,JointCollectionTpl> & model,
  ...
  #define CHECK_DATA(a)  if(!(a)) return false;
      ...
      for(pinocchio::JointIndex joint_id = 1; joint_id < static_cast<pinocchio::JointIndex>(model.njoints); ++joint_id)
      {
