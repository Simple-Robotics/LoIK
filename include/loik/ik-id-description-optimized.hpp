...
namespace loik
{
  ...
  struct IkProblemFormulationOptimized 
  {
      ...
      explicit IkProblemFormulationOptimized(const int nj, const int nb, const int nc_eq, const int eq_c_dim, const int ineq_c_dim) 
          : eq_c_dim_(eq_c_dim),
            nj_(nj),
            nb_(nb),
            nc_eq_(nc_eq),
      ...
      // PINOCCHIO_ALIGNED_STD_VECTOR(Vec6) Aty;         // Ai.transpose * yi  ; updated in the solver after dual variables (yis) udpate
