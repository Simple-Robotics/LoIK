  ...
  struct FirstOrderLoikTpl : IkIdSolverBaseTpl<_Scalar>
  {
    ...
    void FwdPass2()
    {
      ...
      for (const auto & idx : joint_range_)
      {
        ...
        const DVec & ri = ik_id_data_.ris[idx];
        const SE3 & liMi = ik_id_data_.liMi[idx];
        const Motion vi_parent =
          liMi.actInv(ik_id_data_.vis[parent]); // ith joint's parent spatial velocity in
    ...
    void ComputeResiduals()
    {
      ...
      if (this->verbose_)
      {
        ...
        std::cout
          << "primal residual kinematics constraint violation from QP:"
    ...
    void CheckConvergence()
    {
      ...
      if ((this->primal_residual_ < this->tol_primal_) && (this->dual_residual_ < this->tol_dual_))
      {
        ...
        if (this->verbose_)
        {
          std::cerr << "[FirstOrderLoikTpl::CheckConvergence]: converged in " << this->iter_
                    << "iterations !!!" << std::endl;
        }
      }
    };
    ...
    void CheckFeasibility()
    {
      ...
      if (this->verbose_)
      {
        ...
        std::cout << "check lhs kinematics: "
                  << ((problem_.A_qp_.transpose() * problem_.delta_y_qp_).segment(0, 6 * nb_))
                       .template lpNorm<Eigen::Infinity>()
                  << std::endl;
        std::cout << "check lhs slack: "
                  << ((problem_.A_qp_.transpose() * problem_.delta_y_qp_).segment(6 * nb_, nv_))
                       .template lpNorm<Eigen::Infinity>()
                  << std::endl;
      ...
      if (dual_infeasibility_cond_1 && dual_infeasibility_cond_2)
      {
        ...
        if (dual_infeasibility_cond_3 && dual_infeasibility_cond_4)
        {
          ...
          if (this->verbose_)
          {
            std::cerr << "WARNING [FirstOrderLoikTpl::CheckFeasibility]: IK problem "
                         "is dual infeasible !!!"
                      << std::endl;
          }
        }
    ...
    void UpdateMu()
    {
      ...
      {
        // using OSQP strategy
        throw(std::runtime_error("[FirstOrderLoikTpl::UpdateMu]: mu update strategy "
      ...
      {
        throw(
          std::runtime_error("[FirstOrderLoikTpl::UpdateMu]: mu update strategy not supported"));
      }
    };
