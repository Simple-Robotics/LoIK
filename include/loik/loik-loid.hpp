...
struct FirstOrderLoikTpl : IkIdSolverBaseTpl<_Scalar> {
  ...
  struct LoikSolverInfo : Base::SolverInfo {
    explicit LoikSolverInfo(const int max_iter) : Base::SolverInfo(max_iter) {
      primal_residual_kinematics_list_.reserve(
      ...
      tail_solve_primal_residual_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_primal_residual_kinematics_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_primal_residual_task_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_primal_residual_slack_list_.reserve(
          static_cast<std::size_t>(max_iter));

      tail_solve_dual_residual_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_dual_residual_v_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_dual_residual_nu_list_.reserve(
          static_cast<std::size_t>(max_iter));

      tail_solve_delta_x_qp_inf_norm_list_.reserve(
          static_cast<std::size_t>(max_iter));
      tail_solve_delta_z_qp_inf_norm_list_.reserve(
          static_cast<std::size_t>(max_iter));
    };
  ...
  FirstOrderLoikTpl(const int max_iter, const Scalar &tol_abs,
                    ...
                    const int num_eq_c, const int eq_c_dim, const Model &model,
                    IkIdData &ik_id_data, const bool warm_start,
                    const bool verbose, const bool logging)
      : Base(max_iter, tol_abs, tol_rel, tol_primal_inf, tol_dual_inf, rho, mu,
             mu_equality_scale_factor, mu_update_strat, verbose, logging),
        model_(model), ik_id_data_(ik_id_data),
        problem_(model.njoints, model.njoints - 1, num_eq_c, eq_c_dim,
                 model.nv),
        nj_(model.njoints), nb_(model.njoints - 1), nv_(model.nv),
        warm_start_(warm_start), loik_solver_info_(max_iter) {
  ...
  void FwdPass1() {
    ...
    for (const auto &idx : joint_range_) {
      ...
      // build Ris and ris TODO: Ris only need to be computed once for constant
      // mu
      ...
      ik_id_data_.His[idx].noalias() =
          this->rho_ * DMat::Identity(6, 6) + H_ref;

      // PINOCCHIO_EIGEN_MALLOC_NOT_ALLOWED();
      ik_id_data_.pis[idx].noalias() =
          -this->rho_ * ik_id_data_.vis_prev[idx].toVector() -
          H_ref.transpose() * v_ref.toVector();
      // PINOCCHIO_EIGEN_MALLOC_ALLOWED();
    }
    ...
    for (const auto &c_id : problem_.active_task_constraint_ids_) {
      ...
      ik_id_data_.pis[c_id].noalias() +=
          Ai.transpose() * yi - mu_eq_ * Ai.transpose() * bi;
  ...
  void BwdPass() {
    ...
    for (auto it = joint_range_.rbegin(); it != joint_range_.rend(); ++it) {
      ...
      const DMat &Si =
          jdata.S().matrix(); // TODO: this will cause memory allocation
      const DMat &Ri = ik_id_data_.Ris[idx];
      const DVec &ri = ik_id_data_.ris[idx];

      ik_id_data_.Dis[idx].noalias() =
          Ri +
          Si.transpose() * Hi * Si; // TODO: this will cause memory allocation
      ik_id_data_.Di_invs[idx].noalias() =
          ik_id_data_.Dis[idx]
              .inverse(); // TODO: this will cause memory allocation
      const DMat &Di_inv = ik_id_data_.Di_invs[idx];
      ik_id_data_.Pis[idx].noalias() =
          DMat::Identity(6, 6) -
          Hi * Si * Di_inv *
              Si.transpose(); // TODO: this will cause memory allocation
      const Mat6x6 &Pi = ik_id_data_.Pis[idx];

      // ik_id_data_.His[parent].noalias() += liMi.toDualActionMatrix() * (Pi *
      // Hi) * liMi.inverse().toActionMatrix();
      ...
      ik_id_data_.pis[parent].noalias() +=
          ...
          (Pi * pi -
           Hi * Si * Di_inv * ri); // TODO: this will cause memory allocation
    }
  ...
  void BwdPassOptimized() {
    ...
    for (auto it = joint_range_.rbegin(); it != joint_range_.rend(); ++it) {
      ...
      jmodel.calc_aba(jdata.derived(), Ri.diagonal(), Hi, parent > 0);
      ...
      ik_id_data_.pis[parent].noalias() +=
          liMi.toDualActionMatrix() * (pi - jdata.UDinv() * ri);
      // ik_id_data_.Dis[idx].noalias() = Ri + Si.transpose() * Hi * Si; //
      // TODO: this will cause memory allocation
      // ik_id_data_.Di_invs[idx].noalias() = ik_id_data_.Dis[idx].inverse(); //
      // TODO: this will cause memory allocation const DMat& Di_inv =
      // ik_id_data_.Di_invs[idx]; ik_id_data_.Pis[idx].noalias() =
      // DMat::Identity(6, 6) - Hi * Si * Di_inv * Si.transpose();      // TODO:
      // this will cause memory allocation const Mat6x6& Pi =
      // ik_id_data_.Pis[idx];

      // ik_id_data_.His[parent].noalias() += liMi.toDualActionMatrix() * (Pi *
      // Hi) * liMi.inverse().toActionMatrix();
      // ik_id_data_.His[parent].noalias() += liMi.toDualActionMatrix() * (Pi *
      // Hi) * liMi.toActionMatrixInverse(); ik_id_data_.pis[parent].noalias()
      // += liMi.toDualActionMatrix() * (Pi * pi - Hi * Si * Di_inv * ri); //
      // TODO: this will cause memory allocation
    }
  };
  ...
  void FwdPass2() {
    ...
    for (const auto &idx : joint_range_) {
      ...
      const Motion vi_parent = liMi.actInv(
          ik_id_data_.vis[parent]); // ith joint's parent spatial velocity in
                                    // joint i's local frame
      ...
      ik_id_data_.Si_nui_s[idx] =
          Motion(Si * ik_id_data_.nu.segment(joint_idx_v, joint_nv));
      ik_id_data_.vis[idx] = vi_parent + ik_id_data_.Si_nui_s[idx];

      ik_id_data_.fis[idx].noalias() =
          Hi * ik_id_data_.vis[idx].toVector() + pi;
    }
  ...
  void FwdPass2Optimized() {
    ...
    for (const auto &idx : joint_range_) {
      ...
      ik_id_data_.Si_nui_s[idx] =
          Motion(jdata.S() * ik_id_data_.nu.segment(joint_idx_v, joint_nv));
      ik_id_data_.vis[idx] = vi_parent + ik_id_data_.Si_nui_s[idx];
  ...
  void BoxProj() {
    PINOCCHIO_EIGEN_MALLOC_NOT_ALLOWED();
    // update slack
    ik_id_data_.z.noalias() = problem_.ub_.cwiseMin(problem_.lb_.cwiseMax(
        ik_id_data_.nu + (1.0 / mu_ineq_) * ik_id_data_.w));
    PINOCCHIO_EIGEN_MALLOC_ALLOWED();
  };
  ...
  void ComputeResiduals() {
    ...
    this->primal_residual_ =
        primal_residual_vec_.template lpNorm<Eigen::Infinity>();
    primal_residual_task_ = primal_residual_vec_.segment(0, m * nb_)
                                .template lpNorm<Eigen::Infinity>();
    primal_residual_slack_ = primal_residual_vec_.segment(m * nb_, nv_)
                                 .template lpNorm<Eigen::Infinity>();

    // std::cout << "primal_residual_vec: " << primal_residual_vec_.transpose()
    // << std::endl; std::cout << "primal residual check: " <<
    // primal_residual_vec_.template lpNorm<Eigen::Infinity>() << std::endl;

    // DEBUG testing primal kinematics residual
    if (this->verbose_) {
      DVec primal_residual_from_qp =
          problem_.A_qp_ * problem_.x_qp_ - problem_.z_qp_;

      std::cout << "primal residual kinematics constraint violation from QP:"
                << (primal_residual_from_qp.segment(0, 6 * nb_))
                       .template lpNorm<Eigen::Infinity>()
                << std::endl;

      std::cout << "primal residual: " << this->primal_residual_ << std::endl;
      std::cout << "primal residual task: " << primal_residual_task_
                << std::endl;
      std::cout << "primal residual slack: " << primal_residual_slack_
                << std::endl;
    }
    ...
    for (auto it = joint_range_.rbegin(); it != joint_range_.rend(); ++it) {
      ...
      dual_residual_vec_.segment(6 * nb_ + row_start_idx_wi, jmodel.nv())
          .noalias() = Si.transpose() * fi + wi;
    }
    ...
    this->dual_residual_ =
        dual_residual_vec_.template lpNorm<Eigen::Infinity>();
    dual_residual_v_ = (dual_residual_vec_.segment(0, 6 * nb_))
                           .template lpNorm<Eigen::Infinity>();
    dual_residual_nu_ = (dual_residual_vec_.segment(6 * nb_, nv_))
                            .template lpNorm<Eigen::Infinity>();
    ...
    if (this->verbose_) {
      ...
      std::cout << "normInf delta_x_qp: "
                << problem_.delta_x_qp_.template lpNorm<Eigen::Infinity>()
                << std::endl;
      std::cout << "normInf delta_z_qp: "
  ...
  void CheckConvergence() {
    ...
    this->tol_primal_ =
        this->tol_abs_ +
        this->tol_rel_ *
            std::max((problem_.A_qp_ * problem_.x_qp_)
                         .template lpNorm<Eigen::Infinity>(),
    ...
    this->tol_dual_ =
        this->tol_abs_ +
        this->tol_rel_ *
            std::max(std::max((problem_.P_qp_ * problem_.x_qp_)
                                  .template lpNorm<Eigen::Infinity>(),
    ...
    if ((this->primal_residual_ < this->tol_primal_) &&
      ...
      if (this->verbose_) {
        std::cerr << "[FirstOrderLoikTpl::CheckConvergence]: converged in "
  ...
  void CheckFeasibility() {
    ...
    bool primal_infeasibility_cond_1 =
        (problem_.A_qp_.transpose() * problem_.delta_y_qp_)
            .template lpNorm<Eigen::Infinity>() <=
    ...
    if (this->verbose_) {
      std::cout << "****************************CheckFeasibility***************"
                   "*************"
                << std::endl;

      std::cout << "primal_infeasibility_cond_1: "
                << primal_infeasibility_cond_1 << std::endl;
      std::cout << "delta_y_qp normInf: "
                << (problem_.delta_y_qp_).template lpNorm<Eigen::Infinity>()
                << std::endl;
      std::cout << "check lhs: "
                << ((problem_.A_qp_.transpose() * problem_.delta_y_qp_))
                       .template lpNorm<Eigen::Infinity>()
                << std::endl;
      std::cout << "check lhs kinematics: "
                << ((problem_.A_qp_.transpose() * problem_.delta_y_qp_)
                        .segment(0, 6 * nb_))
                       .template lpNorm<Eigen::Infinity>()
                << std::endl;
      std::cout << "check lhs slack: "
                << ((problem_.A_qp_.transpose() * problem_.delta_y_qp_)
                        .segment(6 * nb_, nv_))
                       .template lpNorm<Eigen::Infinity>()
                << std::endl;
      ...
      std::cout << "primal_infeasibility_cond_2: "
                << primal_infeasibility_cond_2 << std::endl;
      std::cout << "check lhs: "
                << (problem_.ub_qp_.transpose() * problem_.delta_y_qp_plus_ +
                    problem_.lb_qp_.transpose() * problem_.delta_y_qp_minus_)
                       .value()
    ...
    if (primal_infeasibility_cond_1 && primal_infeasibility_cond_2) {
      ...
      if (this->verbose_) {
        std::cerr << "WARNING [FirstOrderLoikTpl::CheckFeasibility]: IK "
    ...
    bool dual_infeasibility_cond_1 =
        (problem_.P_qp_ * problem_.delta_x_qp_)
            .template lpNorm<Eigen::Infinity>() <=
        this->tol_dual_inf_ *
            (problem_.delta_x_qp_).template lpNorm<Eigen::Infinity>();
    ...
    if (dual_infeasibility_cond_1 && dual_infeasibility_cond_2) {
      ...
      if (dual_infeasibility_cond_3 && dual_infeasibility_cond_4) {
        ...
        if (this->verbose_) {
          std::cerr
              << "WARNING [FirstOrderLoikTpl::CheckFeasibility]: IK problem "
                 "is dual infeasible !!!"
              << std::endl;
        }
      }
  ...
  void UpdateMu() {
    ...
    if (this->mu_update_strat_ == ADMMPenaltyUpdateStrat::DEFAULT) {
      ...
      throw(std::runtime_error(
          "[FirstOrderLoikTpl::UpdateMu]: mu update strategy "
          "OSQP not yet implemented"));
      ...
      throw(std::runtime_error(
          "[FirstOrderLoikTpl::UpdateMu]: mu update strategy not supported"));
    }
  };
  ...
  ///
  void SolveInit(const DVec &q, const Mat6x6 &H_ref, const Motion &v_ref,
  ...
  /// joint velocities onto the box constraint set
  ///
  void Solve(const DVec &q, const Mat6x6 &H_ref, const Motion &v_ref,
             const std::vector<Index> &active_task_constraint_ids,
             const PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) & Ais,
             const PINOCCHIO_ALIGNED_STD_VECTOR(Vec6) & bis, const DVec &lb,
             const DVec &ub);
