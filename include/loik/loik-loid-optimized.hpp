...
struct LoikBackwardStepVisitor
  ...
  typedef IkIdDataTypeOptimizedTpl<Scalar, Options, JointCollectionTpl>
      IkIdData;
  ...
  algo(const pinocchio::JointModelBase<JointModel> &jmodel,
    ...
    jmodel.calc_aba(jdata.derived(), jmodel.jointVelocitySelector(R), Hi_aba,
                    parent > 0);
...
struct LoikForwardStep2Visitor
  ...
  typedef boost::fusion::vector<const Model &, IkIdData &,
  ...
  algo(const pinocchio::JointModelBase<JointModel> &jmodel,
    ...
    Motion vi_parent = liMi.actInv(
        ik_id_data.vis[parent]); // ith joint's parent spatial velocity in joint
                                 // i's local frame
    ...
    ik_id_data.vis[idx] +=
        jdata.S() * jmodel.jointVelocitySelector(ik_id_data.nu);
    ...
    ik_id_data.fis[idx].linear() = (Hi * ik_id_data.vis[idx].toVector())
                                       .template segment<3>(Force::LINEAR) +
                                   pi.linear();
    ik_id_data.fis[idx].angular() = (Hi * ik_id_data.vis[idx].toVector())
                                        .template segment<3>(Force::ANGULAR) +
                                    pi.angular();

    // update Href_v
    ik_id_data.Href_v[idx].noalias() =
        problem.H_refs_[idx] * ik_id_data.vis[idx].toVector();
...
struct LoikBackwardStep2Visitor
  ...
  typedef boost::fusion::vector<const Model &, IkIdData &,
                                const ProblemFormulation &, ResidualVec &>
      ArgsType;
  ...
  algo(const pinocchio::JointModelBase<JointModel> &jmodel,
       pinocchio::JointDataBase<typename JointModel::JointDataDerived> &jdata,
       const Model &model, IkIdData &ik_id_data,
       const ProblemFormulation &problem, ResidualVec &dual_residual_vec) {
    PINOCCHIO_EIGEN_MALLOC_NOT_ALLOWED();
    ...
    dual_residual_vec.template segment<6>((static_cast<int>(idx) - 1) * 6)
        .noalias() += Href_v - Href_vref + ik_id_data.fis_diff[idx].toVector();
...
struct FirstOrderLoikOptimizedTpl : IkIdSolverBaseTpl<_Scalar> {
  ...
  FirstOrderLoikOptimizedTpl(
      const int max_iter, const Scalar &tol_abs, const Scalar &tol_rel,
      const Scalar &tol_primal_inf, const Scalar &tol_dual_inf,
      const Scalar &rho, const Scalar &mu,
      const Scalar &mu_equality_scale_factor,
      const ADMMPenaltyUpdateStrat &mu_update_strat, const int num_eq_c,
      const int eq_c_dim, const Model &model, IkIdData &ik_id_data,
      const bool warm_start, const bool verbose, const bool logging)
    ...
    // residual vectors
    primal_residual_vec_ =
        std::numeric_limits<Scalar>::infinity() * DVec::Ones(6 * nb_ + nv_);
    dual_residual_vec_ =
        std::numeric_limits<Scalar>::infinity() * DVec::Ones(6 * nb_ + nv_);
  ...
  void FwdPass1() {
    ...
    for (const auto &idx : joint_range_) {
      ...
      // PINOCCHIO_EIGEN_MALLOC_NOT_ALLOWED();
      ik_id_data_.pis[idx].linear() =
          -this->rho_ * ik_id_data_.vis_prev[idx].linear();
      ik_id_data_.pis[idx].angular() =
          -this->rho_ * ik_id_data_.vis_prev[idx].angular();
      ik_id_data_.pis[idx].linear() -= Hv_i.template segment<3>(Motion::LINEAR);
      ik_id_data_.pis[idx].angular() -=
          Hv_i.template segment<3>(Motion::ANGULAR);
  ...
  void BwdPassOptimizedVisitor() {
    typedef internal::LoikBackwardStepVisitor<_Scalar, _Options,
                                              JointCollectionTpl>
        loik_bwd_pass;
  ...
  void FwdPass2OptimizedVisitor() {
    typedef internal::LoikForwardStep2Visitor<_Scalar, _Options,
                                              JointCollectionTpl>
        loik_fwd_pass2;
  ...
  void BoxProj() {
    ...
    primal_residual_vec_.segment(6 * nb_, nv_).noalias() =
        ik_id_data_.nu - ik_id_data_.z;
  ...
  void DualUpdate() {
    ...
    for (const auto &c_id : problem_.active_task_constraint_ids_) {
      ...
      primal_residual_vec_.template segment<6>(6 * (static_cast<int>(c_id) - 1))
          .noalias() = ik_id_data_.Av_minus_b[c_id];

      // initialize dual residual vector segment corresponding to c_id
      dual_residual_vec_.template segment<6>((static_cast<int>(c_id) - 1) * 6)
          .noalias() = ik_id_data_.Aty[c_id];
  ...
  void BwdPass2OptimizedVisitor() {
    typedef internal::LoikBackwardStep2Visitor<_Scalar, _Options,
                                               JointCollectionTpl>
        loik_bwd_pass2;
    for (Index i = (Index)model_.njoints - 1; i > 0; --i) {
      loik_bwd_pass2::run(
          model_.joints[i], ik_id_data_.joints[i],
          typename loik_bwd_pass2::ArgsType(model_, ik_id_data_, problem_,
                                            dual_residual_vec_));
    }
  ...
  void ComputePrimalResiduals() {
    ...
    primal_residual_task_ = primal_residual_vec_.segment(0, 6 * nb_)
                                .template lpNorm<Eigen::Infinity>();
    primal_residual_slack_ = primal_residual_vec_.segment(6 * nb_, nv_)
                                 .template lpNorm<Eigen::Infinity>();
  ...
  void CheckConvergence(){
      ...
      //                                               * problem_.y_qp_).template lpNorm<Eigen::Infinity>()),
