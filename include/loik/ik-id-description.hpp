...
namespace loik
{
  ...
  struct IkProblemFormulation
  {
    ...
    , ineq_c_dim_(ineq_c_dim)
    {
      ...
      {
        throw(
          std::runtime_error("[IkProblemFormulation::IkProblemFormulation]: nb does not equal to "
      ...
      const PINOCCHIO_ALIGNED_STD_VECTOR(Motion) & v_refs)
    {
      ...
      {
        throw(std::runtime_error("[IkProblemFormulation::UpdateReferences]: input "
      ...
      const PINOCCHIO_ALIGNED_STD_VECTOR(Vec6) & bis)
    {
      ...
      {
        throw(
          std::runtime_error("[IkProblemFormulation::UpdateEqConstraints]: task_constraint_ids, "
    ...
    void UpdateEqConstraint(const Index c_id, const Mat6x6 & Ai, const Vec6 & bi)
    {
      ...
      {
        throw(
          std::runtime_error("[IkProblemFormulation::UpdateEqConstraint]: constraint doesn't yet "
                             "exist at link 'c_id' !!! "));
      }
  ...
  struct IkProblemStandardQPFormulation : IkProblemFormulation<_Scalar>
  {
      ...
      const IkIdData & ik_id_data)
    {
      ...
      for (Index idx : joint_range)
      {
        ...
        {
          throw(
            std::runtime_error("[IkProblemStandardQPFormulation::UpdateQPADMMSolveInit]: parent "
