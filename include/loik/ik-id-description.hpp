...
template <typename _Scalar> struct IkProblemFormulation {
  ...
  explicit IkProblemFormulation(const int nj, const int nb, const int nc_eq,
                                const int eq_c_dim, const int ineq_c_dim)
...
struct IkProblemStandardQPFormulation : IkProblemFormulation<_Scalar> {
  ...
  explicit IkProblemStandardQPFormulation(const int nj, const int nb,
                                          const int nc_eq, const int eq_c_dim,
                                          const int ineq_c_dim)
      : Base(nj, nb, nc_eq, eq_c_dim, ineq_c_dim) {
    qp_constraint_dim_ =
        6 * this->nb_ + this->eq_c_dim_ * this->nb_ + this->ineq_c_dim_;
  ...
  ///
  /// \brief this should only be called to completely reset the problem
  /// formulation, it will set all quantities to zero,
  ///        but problem dimensions and number of bodies and constraints won't
  ///        change
  ///
  void Reset() {
    Base::Reset();
  ...
  ///
  /// \brief problem formulation update function, should be called before runing
  /// ADMM iterations
  ///        should only be called once per ADMM solve() call, used to update
  ///        equality constraints and tracking references
  ///
  void
  UpdateQPADMMSolveInit(const Mat6x6 &H_ref, const Motion &v_ref,
                        const std::vector<Index> &active_task_constraint_ids,
                        const PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) & Ais,
                        const PINOCCHIO_ALIGNED_STD_VECTOR(Vec6) & bis,
                        const DVec &lb, const DVec &ub, const Model &model,
    ...
    lb_qp_.segment(6 * this->nb_ + this->eq_c_dim_ * this->nb_,
                   this->ineq_c_dim_) = this->lb_;
    ub_qp_.segment(6 * this->nb_ + this->eq_c_dim_ * this->nb_,
                   this->ineq_c_dim_) = this->ub_;

    A_qp_.block(0, 0, 6 * this->nb_, 6 * this->nb_) =
        -1.0 * DMat::Identity(6 * this->nb_, 6 * this->nb_);
    A_qp_.block(6 * this->nb_ + this->eq_c_dim_ * this->nb_, 6 * this->nb_,
                this->ineq_c_dim_, this->ineq_c_dim_) =
    ...
    for (Index idx : joint_range) {
      ...
      P_qp_.block(row_start_idx_Hi_vi, col_start_idx_Hi_vi, 6, 6).noalias() =
          this->H_refs_[idx];
      ...
      if (parent > 0) {
        ...
        A_qp_.block(row_start_idx_Si, col_start_idx_i, 6, 6).noalias() =
            -Mat6x6::Identity();
      } else if (parent == 0) {
        A_qp_.block(row_start_idx_Si, col_start_idx_i, 6, 6).noalias() =
            -Mat6x6::Identity();
        ...
        throw(std::runtime_error(
            "[IkProblemStandardQPFormulation::UpdateQPADMMSolveInit]: parent "
    ...
    for (Index c_idx : this->active_task_constraint_ids_) {
      ...
      int row_start_idx_Ai =
          6 * this->nb_ + (static_cast<int>(c_idx) - 1) * this->eq_c_dim_;
      int col_start_idx_Ai = (static_cast<int>(c_idx) - 1) * 6;

      A_qp_.block(row_start_idx_Ai, col_start_idx_Ai, this->eq_c_dim_, 6)
          .noalias() = this->Ais_[c_idx_in_vec];

      // fill in the bis from motion constraints into ub_qp_ and lb_qp_
      int row_start_idx_boundi =
          6 * this->nb_ + (static_cast<int>(c_idx) - 1) * this->eq_c_dim_;

      lb_qp_.segment(row_start_idx_boundi, this->eq_c_dim_).noalias() =
          this->bis_[c_idx_in_vec];
      ub_qp_.segment(row_start_idx_boundi, this->eq_c_dim_).noalias() =
          this->bis_[c_idx_in_vec];
  ...
  void UpdateQPADMMSolveLoop(const IkIdData &ik_id_data) {
    ...
    for (Index idx : ik_id_data.joint_range) {
      ...
      int row_start_idx_yi =
          6 * this->nb_ + (static_cast<int>(idx) - 1) * this->eq_c_dim_;

      // fill x_qp_
      x_qp_.segment(row_start_idx_vi, 6).noalias() =
          ik_id_data.vis[idx].toVector(); // vi

      // fill dual variable y_qp associated with all constraints
      y_qp_.segment(row_start_idx_fi, 6).noalias() = ik_id_data.fis[idx]; // fi
      y_qp_.segment(row_start_idx_yi, this->eq_c_dim_).noalias() =
          ik_id_data.yis[idx]; // yi

      // std::cout << "=============================" << std::endl;
      // std::cout << "idx: " << idx << std::endl;
      // std::cout << "vi: " << ik_id_data.vis[idx].toVector().transpose() <<
      // std::endl; std::cout << "fi: " << ik_id_data.fis[idx].transpose() <<
      // std::endl; std::cout << "yi: " << ik_id_data.yis[idx].transpose() <<
      // std::endl;
    }
    ...
    y_qp_
        .segment(6 * this->nb_ + this->nb_ * this->eq_c_dim_, this->ineq_c_dim_)
        .noalias() = ik_id_data.w;

    // fill part of z_qp_ related to inequality slack
    z_qp_
        .segment(6 * this->nb_ + this->eq_c_dim_ * this->nb_, this->ineq_c_dim_)
        .noalias() = ik_id_data.z;
    ...
    // std::cout << "normInf delta_x_qp_: " << delta_x_qp_.template
    // lpNorm<Eigen::Infinity>() << std::endl; std::cout << "normInf
    // delta_y_qp_: " << delta_y_qp_.template lpNorm<Eigen::Infinity>() <<
    // std::endl; std::cout << "normInf delta_z_qp_: " << delta_z_qp_.template
    // lpNorm<Eigen::Infinity>() << std::endl; std::cout << "delta_x_qp_: " <<
    // delta_x_qp_.transpose() << std::endl; std::cout << "delta_y_qp_: " <<
    // delta_y_qp_.transpose() << std::endl; std::cout << "delta_z_qp_: " <<
    // delta_z_qp_.transpose() << std::endl;
  ...
  DVec delta_y_qp_plus_; // copy of 'delta_y_qp_' with all negative coefficients
                         // set to 0
  DVec delta_y_qp_minus_; // copy of 'delta_y_qp_' with all positive
