...
namespace loik
{
  ...
  struct IkProblemFormulation 
  {
      ...
      explicit IkProblemFormulation(const int nj, const int nb, const int nc_eq, const int eq_c_dim, const int ineq_c_dim) 
          : eq_c_dim_(eq_c_dim),
            nj_(nj),
            nb_(nb),
            nc_eq_(nc_eq),
      ...
      {   
          if (nj_ != nb_ + 1) {
              throw(std::runtime_error("[IkProblemFormulation::IkProblemFormulation]: nb does not equal to nj - 1, robot model not supported !!!"));
          }

          if (eq_c_dim != 6) {
              throw(std::runtime_error("[IkProblemFormulation::IkProblemFormulation]: equality constraint dimension is not 6, problem formulation not supported !!!"));
          }
      ...
      {   
          ...
          if (H_refs_.size() <= 0) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateReference]: 'H_refs_' and 'v_refs_' are empty"));
          }
      ...
      ///
      void UpdateReferences(const PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6)& H_refs, const PINOCCHIO_ALIGNED_STD_VECTOR(Motion)& v_refs) 
      {
          if (H_refs.size() != static_cast<std::size_t>(nj_) || v_refs.size() != static_cast<std::size_t>(nj_)) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateReferences]: input arguments 'H_refs', 'v_refs' have wrong size!!"));
          }
          H_refs_ = H_refs;
          v_refs_ = v_refs;
      };
      ...
      ///
      /// \brief batch update equality constraints, number of equality constraints and constraint dimensions must not change
      ///
      ...
      {   
          // check constraint specification is consistant
            if ( ! ((active_task_constraint_ids.size() == Ais.size()) && ((Ais.size() == bis.size()))) ) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraints]: task_constraint_ids, Ais, and bis have different size !!!"));
            }

          // check constraint dimension is consistent between input arguments! 
          if ( Ais[0].rows() != bis[0].rows()) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraints] Ai and bi dimension mismatch!!"));
          }

          // check if number of equality constraints has changed
          if (active_task_constraint_ids.size() != static_cast<std::size_t>(nc_eq_)) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraints]: number of equality constraints doesn't match initialization!!!"));
          ...
          if (Ais[0].rows() != eq_c_dim_) {
              ...
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraints]: equality constraint dimension has changed!!! Updating constraint dimension" ));
          }
      ...
      ///
      /// \brief set equality constraints by individual link id and updating 'Ai' and 'bi', the link id to update must already be 
      ...
      void UpdateEqConstraint(const Index c_id, const Mat6x6& Ai, const Vec6& bi)
      {
          ...
          auto found_it = std::find(active_task_constraint_ids_.begin(), active_task_constraint_ids_.end(), c_id);

          // if 'c_id' not present in 'active_task_constraint_ids_', invoke 'AddEqConstraint()'
          if ( found_it == active_task_constraint_ids_.end() ) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraint]: constraint doesn't yet exist at link 'c_id' !!! "));
          }

          // check if constraint dimension is consistent 
          if (Ai.rows() != bi.size() || Ai.rows() != static_cast<std::size_t>(eq_c_dim_)) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraint]: constraint dimension inconsistent!!!"));
          }

          // count how many times 'c_id' appear in 'active_task_constraint_ids_'
          int c_id_count = std::count(active_task_constraint_ids_.begin(), active_task_constraint_ids_.end(), c_id);
          
          // if 'c_id' appear more than once, then something went wrong, throw. 
          if (c_id_count > 1) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateEqConstraint]: multiple constraint specification for the same link id, not supported, terminating !!!"));
          }
      ...
      ///
      /// \brief update equality constriant by link id, update 'bi' only, keep using old 'Ai', constraint must already be present for link id
      ///
      ...
      ///
      /// \brief (deactivated for now) add equality constraint by link id. If constraint is already present for link id, then invoke 'UpdateEqConstraint()'
      ///
      void AddEqConstraint(const Index c_id, const Mat6x6& Ai, const Vec6& bi) 
      {
          ...
          if ( found_it != active_task_constraint_ids_.end() ) {
              ...
              if (Ai.rows() != bi.rows()) {
                  throw(std::runtime_error("[IkProblemFormulation::AddEqConstraint]: input arguments constraint dimension inconsistent !!!"));
              }

              if (Ai.rows() != static_cast<std::size_t>(eq_c_dim_)) {
                  throw(std::runtime_error("[IkProblemFormulation::AddEqConstraint]: input constraint dimension differ from existing constriant dimension!!!"));
              }
      ...
      ///
      /// \brief (deactivated for now) remove equality constraint by link id, if constraint not already present for link id, then do nothing 
      ///
      void RemoveEqConstraint(const Index c_id) 
      {
          ...
          if ( found_it == active_task_constraint_ids_.end() ) {
              ...
              std::cerr << "WARNING [IkProblemFormulation::RemoveEqConstraint]: no constraint defined at link id, nothing to remove. " << std::endl;
              return;
          }
      ...
      void UpdateIneqConstraints(const DVec& lb, const DVec& ub)
      {
          ...
          if (lb.size() != ub.size()) {
              throw(std::runtime_error("[IkProblemFormulation::UpdateIneqConstraints]: lower bound and upper bound have different dimensions!!!"));
          }

          // check is inequality constraint dimension has changed 
          if (lb.size() != ineq_c_dim_) {
              throw(std::runtime_error("IkProblemFormulation::UpdateIneqConstraints]: inequality constraint dimension has changed, this is not supported currently!!!"));
          }
      ...
      int eq_c_dim_;                                // constraint dimension of the equality task constraint, this is shared across all eq constraints
      ...
      PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) H_refs_; // weights for spatial velocity reference tracking in the cost, i.e. soft constraints
      PINOCCHIO_ALIGNED_STD_VECTOR(Motion) v_refs_; // spatial velocity tracking references 
      std::vector<Index> active_task_constraint_ids_; // vector of idx of links with active hard constraints

      PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) Ais_;      // 'A' terms in the active equality constraint definitions   
      PINOCCHIO_ALIGNED_STD_VECTOR(Vec6) bis_;      // 'b' terms in the active equality constraint definitions
    ...
    : IkProblemFormulation<_Scalar>
  {
      ...
      explicit IkProblemStandardQPFormulation(const int nj, const int nb, const int nc_eq, const int eq_c_dim,  const int ineq_c_dim) 
      ...
      ///
      /// \brief this should only be called to completely reset the problem formulation, it will set all quantities to zero, 
      ...
      ///
      void UpdateQPADMMSolveInit(const Mat6x6& H_ref, const Motion& v_ref, 
                            const std::vector<Index>& active_task_constraint_ids, 
                            const PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6)& Ais,
                            const PINOCCHIO_ALIGNED_STD_VECTOR(Vec6)& bis, 
                            const DVec& lb, const DVec& ub, 
                            const Model& model, const IkIdData& ik_id_data)
      {
          ...
          A_qp_.block(0, 0, 6 * this->nb_, 6 * this->nb_) = -1.0 * DMat::Identity(6 * this->nb_, 6 * this->nb_);
          A_qp_.block(6 * this->nb_ + this->eq_c_dim_ * this->nb_,  6 * this->nb_, this->ineq_c_dim_, this->ineq_c_dim_) 
              = DMat::Identity(this->ineq_c_dim_, this->ineq_c_dim_);
          ...
          for (Index idx : joint_range) {
              ...
              q_qp_.segment(row_start_idx_Hi_vi, 6).noalias() = - (this->H_refs_[idx]).transpose() * (this->v_refs_[idx]).toVector();
              ...
              A_qp_.block(row_start_idx_Si, col_start_idx_Si, 6, joint_nv).noalias() = ik_id_data.joints[idx].S().matrix();
              ...
              if (parent > 0) {
                  ...
                  //                                                                    |<------------------ iMo  ------------------>|   |<--------------- oMp --------------->|
                  A_qp_.block(row_start_idx_Si, col_start_idx_parent, 6, 6).noalias() = ik_id_data.oMi[idx].inverse().toActionMatrix() * ik_id_data.oMi[parent].toActionMatrix();
                  A_qp_.block(row_start_idx_Si, col_start_idx_i, 6, 6).noalias() = - Mat6x6::Identity();
              } else if (parent == 0) {
                  A_qp_.block(row_start_idx_Si, col_start_idx_i, 6, 6).noalias() = -Mat6x6::Identity();
              } else {
                  throw(std::runtime_error("[IkProblemStandardQPFormulation::UpdateQPADMMSolveInit]: parent id < 0, this can't happen !!!"));
              }
          ...
          for (Index c_idx : this->active_task_constraint_ids_) {
              ...
              A_qp_.block(row_start_idx_Ai, col_start_idx_Ai, this->eq_c_dim_, 6).noalias() = this->Ais_[c_idx_in_vec];
          ...
          z_qp_.segment(6 * this->nb_, this->eq_c_dim_ * this->nb_).noalias() = ub_qp_.segment(6 * this->nb_, this->eq_c_dim_ * this->nb_);
      ...
      void UpdateQPADMMSolveLoop(const IkIdData& ik_id_data)
      {
          ...
          y_qp_.segment(6 * this->nb_ + this->nb_ * this->eq_c_dim_, this->ineq_c_dim_).noalias() = ik_id_data.w;

          // fill part of z_qp_ related to inequality slack 
          z_qp_.segment(6 * this->nb_ + this->eq_c_dim_ * this->nb_, this->ineq_c_dim_).noalias() = ik_id_data.z;
