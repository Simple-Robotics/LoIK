...
/////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////
template <typename _Scalar> void FirstOrderLoikTpl<_Scalar>::ResetSolver() {
...
void FirstOrderLoikTpl<_Scalar>::SolveInit(
    const DVec &q, const Mat6x6 &H_ref, const Motion &v_ref,
  ...
  problem_.UpdateQPADMMSolveInit(H_ref, v_ref, active_task_constraint_ids, Ais,
                                 bis, lb, ub, model_, ik_id_data_);
}


template <typename _Scalar> void FirstOrderLoikTpl<_Scalar>::Solve() {
  ...
  for (int i = 1; i < this->max_iter_; i++) {
    ...
    if (this->converged_) {
      ...
      if (this->verbose_) {
        std::cerr << "WARNING [FirstOrderLoikTpl::Solve]: primal infeasibility "
      ...
      if (this->verbose_) {
        std::cerr << "WARNING [FirstOrderLoikTpl::Solve]: dual infeasibility "
                     "detected at iteration: "
                  << this->iter_ << std::endl;
      }
...
void FirstOrderLoikTpl<_Scalar>::InfeasibilityTailSolve() {
  ...
  while (problem_.delta_x_qp_.template lpNorm<Eigen::Infinity>() >= 1e-2 ||
    ...
    if (this->verbose_) {
      if (this->iter_ >= this->max_iter_) {
        std::cerr << "WARNING [FirstOrderLoikTpl::InfeasibilityTailSolve]: "
    ...
    if (this->logging_) {
      ...
      loik_solver_info_.tail_solve_primal_residual_task_list_.push_back(
          primal_residual_task_);
      loik_solver_info_.tail_solve_primal_residual_slack_list_.push_back(
          primal_residual_slack_);
      loik_solver_info_.tail_solve_primal_residual_list_.push_back(
          this->primal_residual_);
      loik_solver_info_.tail_solve_dual_residual_nu_list_.push_back(
          dual_residual_nu_);
      loik_solver_info_.tail_solve_dual_residual_v_list_.push_back(
          dual_residual_v_);
      loik_solver_info_.tail_solve_dual_residual_list_.push_back(
          this->dual_residual_);
  ...
  if (this->verbose_) {
    std::cerr << "[FirstOrderLoikTpl::InfeasibilityTailSolve]: tail solve "
                 "completed after "
              << tail_solve_iter_ << " iterations." << std::endl;
    std::cerr
        << "[FirstOrderLoikTpl::InfeasibilityTailSolve]: normInf delta_x_qp_: "
        << problem_.delta_x_qp_.template lpNorm<Eigen::Infinity>() << std::endl;
    std::cerr
        << "[FirstOrderLoikTpl::InfeasibilityTailSolve]: normInf delta_z_qp_: "
