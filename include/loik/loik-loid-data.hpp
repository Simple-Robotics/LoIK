...
namespace loik
{
  ...
  , pinocchio::NumericalBase<IkIdDataTpl<_Scalar, _Options, JointCollectionTpl>>
  {
    ...
    /// \brief temporary variable that stores the link's spatial velocity caused by it's parent joint's motion, i.e. Si * q_dot_i
    ...
    /// \brief "state-feedback" matrix term in the recursion hyputhesis for f_i, analogous to articulated body inertia in [ABA]
    PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) His_aba;

    /// \brief initialized to 'Hi_inertias' but acts container for tmp quantities that gets overwritten during LOIK bwdpass
    ...
    /// \brief intermediate recurssion term, stores each "articulated body inertia" transformed into the motion subspace of the parent joint
    ...
    /// \brief quadratic penalty term associated with nu_i introduced by inequality constraints on nu_i through ADMM
    PINOCCHIO_ALIGNED_STD_VECTOR(DMat) Ris;

    /// \brief affine terms associated with nu_i introduced by inequality constraints on nu_i through ADMM
