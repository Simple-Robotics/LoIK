...
// typedef IkIdDataTpl<pinocchio::context::Scalar, pinocchio::context::Options>
//     IkIdData;

// IkIdDataTpl definition
template <typename _Scalar, int _Options,
          template <typename S, int> class JointCollectionTpl>
struct IkIdDataTpl : pinocchio::serialization::Serializable<
                         IkIdDataTpl<_Scalar, _Options, JointCollectionTpl>>,
  ...
  typedef pinocchio::JointModelTpl<Scalar, Options, JointCollectionTpl>
      JointModel;
  typedef pinocchio::JointDataTpl<Scalar, Options, JointCollectionTpl>
      JointData;
  ...
  /// \brief temporary variable that stores the link's spatial velocity caused
  /// by it's parent joint's motion, i.e. Si * q_dot_i
  ...
  /// analogous to articulated body inertia in [ABA]
  PINOCCHIO_ALIGNED_STD_VECTOR(Mat6x6) His_aba;

  /// \brief initialized to 'Hi_inertias' but acts container for tmp quantities
  /// that gets overwritten during LOIK bwdpass
  ...
  /// \brief intermediate recurssion term, stores each "articulated body
  /// inertia" transformed into the motion subspace of the parent joint
...
inline bool checkIkIdData(
  ...
  std::cout << (static_cast<int>(ik_id_data.joints.size()) == model.njoints)
            << std::endl;
...
#define CHECK_DATA(a)                                                          \
  ...
  CHECK_DATA(static_cast<int>(ik_id_data.joint_full_range.size()) ==
             model.njoints);
  CHECK_DATA(static_cast<int>(ik_id_data.joint_range.size()) ==
             model.njoints - 1);
  ...
  for (typename IkIdData::Index idx = 0; static_cast<int>(idx) < model.njoints;
       idx++) {
